...
import { Download, Calendar, Loader2, FileSpreadsheet, FileText, FileJson, ChevronDown, Search, Filter, ChevronUp, X } from 'lucide-react';
...
const AssessmentReports = () => {
    ...
    const downloadPDF = () => {
        ...
        const tableData = filteredAssessments.map((assessment, index) => [
            ...
            assessment.questions ? assessment.questions.length : (assessment.question_count || 0)
        ]);

        autoTable(doc, {
            head: [['S.No', 'Assessment Title', 'Subject', 'Type', 'Mode', 'Start Date', 'End Date', 'Student Attempt', 'Questions']],
            body: tableData,
            startY: 25,
            theme: 'grid',
    ...
    const downloadExcel = () => {
        const excelData = filteredAssessments.map((assessment, index) => ({
            ...
            "Student Attempt": assessment.attempt_count || 0,
            "Total Questions": assessment.questions ? assessment.questions.length : (assessment.question_count || 0)
        }));
    ...
    const downloadCSV = () => {
        ...
        const headers = ["S.No,Assessment Title,Subject,Type,Mode,Start Date,End Date,Student Attempt,Total Questions"];
        ...
        <div className="p-4 sm:p-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                <h2 className="text-lg sm:text-xl font-bold text-slate-800">
                    Assessment Report <span className="text-xs sm:text-sm font-normal text-slate-500 ml-1 sm:ml-2">({filteredAssessments.length})</span>
                </h2>

                <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                    <button
                        onClick={() => setShowFilters(!showFilters)}
                        className="flex-1 sm:flex-none flex items-center justify-center px-3 sm:px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-lg hover:bg-slate-50 shadow-sm transition-all"
                    >
                    ...
                    <div className="relative flex-1 sm:flex-none w-full sm:w-auto" ref={exportMenuRef}>
                        ...
                        {showExportMenu && (
                            <div className="absolute right-0 mt-2 w-48 bg-white border border-slate-200 rounded-lg shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                                <div className="py-1">
                                    <button onClick={downloadExcel} className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-green-50 hover:text-green-700 flex items-center gap-3 transition-colors">
                                        <FileSpreadsheet className="w-4 h-4 text-green-600" />
                                        <span>Excel (.xlsx)</span>
                                    </button>
                                    <button onClick={downloadCSV} className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-blue-50 hover:text-blue-700 flex items-center gap-3 transition-colors">
                                        <FileText className="w-4 h-4 text-blue-600" />
                                        <span>CSV (.csv)</span>
                                    </button>
                                    <button onClick={downloadPDF} className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-red-50 hover:text-red-700 flex items-center gap-3 transition-colors">
            ...
            {showFilters && (
                <div className="mb-6 p-5 bg-white rounded-2xl shadow-md border border-slate-200/60">
            ...
            {/* Table View */}
            <div className="hidden md:block bg-white rounded-2xl shadow-md border border-slate-200/60 overflow-hidden mb-6">
                <table className="w-full text-sm">
                    <thead className="table-header">
                        <tr>
                            ...
                            <th className="px-4 py-3 text-center">Student Attempt</th>
                        ...
                        {loading ? (
                            ...
                            currentEntries.map((assessment, index) => (
                                <tr key={assessment.assessment_id || index} className="hover:bg-blue-50 transition-colors duration-150">
                                    ...
                                    <td className="px-4 py-3 text-slate-600">{assessment.subject_name || '-'}</td>
                                    <td className="px-4 py-3 text-slate-600">
                                        {assessment.type === 'STANDARD' ? 'Standard' : assessment.type === 'RUBRIC' ? 'Rubric' : assessment.type || '-'}
                                    </td>
                                    <td className="px-4 py-3 text-slate-600">{assessment.mode || '-'}</td>
                                    <td className="px-4 py-3 text-slate-600">{formatDate(assessment.test_start_datetime)}</td>
                                    <td className="px-4 py-3 text-slate-600">{formatDate(assessment.test_end_datetime)}</td>
                                    <td className="px-4 py-3 text-center text-slate-700 font-medium">
                                        {assessment.attempt_count || 0}
                                    </td>
                                    <td className="px-4 py-3 text-center">{assessment.questions ? assessment.questions.length : (assessment.question_count || 0)}</td>
                                </tr>
                            ))
                        ) : (
                            <tr>
                                <td colSpan={9} className="p-12 text-center text-slate-500 bg-slate-50">
            ...
            <div className="md:hidden space-y-4 mb-6">
                {loading ? (
                    ...
                    currentEntries.map((assessment, index) => (
                        <div key={assessment.assessment_id || index} className="bg-white rounded-xl shadow-sm border border-slate-100 p-4 hover:shadow-md transition-shadow">
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex-1">
                                    <h3 className="font-bold text-slate-800 leading-snug">{assessment.title || '-'}</h3>
                                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mt-0.5">{assessment.subject_name || 'No Subject'}</p>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-y-3 mt-4 text-xs">
                                <div>
                                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">Type</p>
                                    <p className="text-slate-700 font-medium truncate">
                                        {assessment.type === 'STANDARD' ? 'Standard' : assessment.type === 'RUBRIC' ? 'Rubric' : assessment.type || '—'}
                                    </p>
                                </div>
                                <div>
                                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">Mode</p>
                                    <p className="text-slate-700 font-medium">{assessment.mode || '—'}</p>
                                </div>
                                <div>
                                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">Start Date</p>
                                    <p className="text-slate-700 font-medium text-xs">{formatDate(assessment.test_start_datetime)}</p>
                                </div>
                                <div>
                                    <p className="text-[10px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">End Date</p>
                                    <p className="text-slate-700 font-medium text-xs">{formatDate(assessment.test_end_datetime)}</p>
                                </div>
                            </div>
                            <div className="mt-4 pt-3 border-t border-slate-50 flex justify-between items-center">
                                <div className="flex items-center text-slate-500 text-[10px]">
                                    <Calendar className="w-3 h-3 mr-1" />
                                    <span>Questions: {assessment.questions ? assessment.questions.length : (assessment.question_count || 0)}</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="p-12 text-center text-slate-500 bg-white rounded-xl border border-dashed border-slate-200">
                        {filters?.academicYear?.length ? "No assessment records found." : "Please select filters to view assessments."}
                    </div>
                )}
            </div>
            ...
            {!loading && filteredAssessments.length > 0 && (
                <div className="flex flex-col sm:flex-row justify-between items-center px-6 py-4 bg-white rounded-2xl shadow-md border border-slate-200/60 text-sm text-slate-600 gap-4">
                    <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-start">
                        <button
                            onClick={handlePrev}
                            disabled={currentPage === 1}
                            className={`flex items-center gap-1 px-4 py-2 rounded-lg font-medium transition-all ${currentPage === 1 ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm'}`}
                        >
                            Previous
                        </button>

                        <span className="text-slate-700 font-medium bg-slate-50 px-3 py-1.5 rounded-lg border border-slate-200 sm:hidden">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-1 px-4 py-2 rounded-lg font-medium transition-all ${currentPage === totalPages ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700 shadow-sm'}`}
                        >
                            Next
                        </button>
                    </div>

                    <span className="text-slate-700 font-medium bg-slate-50 px-4 py-2 rounded-lg border border-slate-200 hidden sm:inline-block">
