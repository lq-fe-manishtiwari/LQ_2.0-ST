...
export default function AddNewAssessment() {
  ...
  // Check if assessment has started and block editing
  React.useEffect(() => {
    if (state.assessmentData) {
      const startTime = new Date(state.assessmentData.test_start_datetime).getTime();
      const currentTime = new Date().getTime();
      if (currentTime >= startTime || state.assessmentData.status === 'Completed') {
        setAlert(
          <SweetAlert
            warning
            confirmBtnText="Back"
