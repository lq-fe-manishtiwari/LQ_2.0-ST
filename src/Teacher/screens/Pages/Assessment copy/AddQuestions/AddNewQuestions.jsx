...
const AddNewQuestion = () => {
  ...
  const goBack = () => navigate("/admin-assessment/questions");
  ...
  const handleSubmit = async (values, { setSubmitting, resetForm, setErrors }) => {
    ...
    if (activeTabIndex === 0) {
      for (let i = 0; i < validQuestions.length; i++) {
        ...
        const emptyOption = labels.find(label => !q[`option${label}`]?.trim());
        if (emptyOption) {
          openWarningModal(
            true,
            `Question ${i + 1}: Option ${emptyOption} is empty. Please fill all ${count} options before saving.`
          );
          setSubmitting(false);
          return;
        }
