...
const RubricEvaluation = ({ rubrics, initialRubricId }) => {
    ...
    if (!rubrics.length || !selectedRubric) {
        return (
    ...
    const maxScore = () => {
        ...
        if (selectedRubric.type === RUBRIC_TYPES.HOLISTIC) {
            const performanceLevels = selectedRubric.levels || selectedRubric.performance_levels || [];
            return Math.max(...performanceLevels.map(l => l.score || l.points || 0), 0);
        }
        if (selectedRubric.type === RUBRIC_TYPES.DEVELOPMENTAL) {
            return (selectedRubric.items || selectedRubric.portfolios || []).length;
        }
        return 0;
    };
        ...
        <div className="flex flex-col lg:flex-row gap-6 h-auto lg:h-[calc(100vh-140px)]">
            <div className="w-full lg:w-1/3 flex flex-col gap-6 order-2 lg:order-1">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex-1">
                    <h3 className="font-bold text-slate-400 text-xs tracking-wider mb-4">SELECT RUBRIC</h3>
                    <select
                    ...
                    <div className="mt-8">
                        ...
                        <div className="bg-slate-50 border border-slate-200 rounded-lg p-4 h-64 flex items-center justify-center text-slate-400 text-sm italic">
                            (Student document or link would appear here)
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full lg:w-2/3 flex flex-col order-1 lg:order-2 h-auto lg:h-[80vh] lg:overflow-hidden bg-white rounded-xl shadow-lg border border-gray-100 p-6 relative">
                ...
                <div className="flex-1 overflow-y-auto pr-2 space-y-6">
                    {selectedRubric.type === RUBRIC_TYPES.ANALYTIC && getCriteria().map((criterion) => (
                        <div key={criterion.criterion_id || criterion.id} className="mb-8">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
                                {(criterion.levels || criterion.performance_levels || []).sort((a, b) => (b.score || b.points || 0) - (a.score || a.points || 0)).map((level) => {
                                    ...
                                    const levelLabel = level.label || level.level_name || 'Level';
                                    const levelId = level.id || level.level_id || levelLabel;
                                    const criterionId = criterion.criterion_id || criterion.id;

                                    return (
                                        <button
                                            key={levelId}
                                            onClick={() => handleScore(criterionId, levelScore)}
                                            className={`p-3 rounded-lg border-2 text-left transition-all relative ${scores[criterionId] === levelScore
                                                ? 'border-blue-500 bg-blue-50 ring-2 ring-blue-200'
                                                : 'border-slate-100 hover:border-blue-200 hover:bg-slate-50'
                                                }`}
                                        >
                                            <div className="flex justify-between items-center mb-1">
                                                <span className={`font-bold text-sm ${scores[criterionId] === levelScore ? 'text-blue-700' : 'text-slate-700'}`}>
                                                    {levelLabel}
                                                </span>
                            ...
                            {getPerformanceLevels()
                                ...
                                .map((level) => {
                                    ...
                                    return (
                                        <button
                                            key={levelId}
                                            onClick={() => handleScore('holistic', levelScore)}
                                            className={`w-full flex items-center gap-4 p-4 rounded-xl border-2 transition-all ${scores['holistic'] === levelScore
                                                ? 'border-orange-500 bg-orange-50 ring-2 ring-orange-200'
                                                : 'border-slate-100 hover:border-orange-200 hover:bg-orange-50/30'
                                                }`}
                                        >
                                            <div className={`w-16 h-16 rounded-full flex items-center justify-center font-bold text-2xl shrink-0 ${scores['holistic'] === levelScore ? 'bg-orange-500 text-white shadow-lg' : 'bg-white border-2 border-slate-100 text-slate-300'
                                                }`}>
                                                {levelScore}
                                            </div>
                                            <div className="text-left">
                                                <h4 className={`font-bold text-lg ${scores['holistic'] === levelScore ? 'text-orange-900' : 'text-slate-700'}`}>
                                                    {levelLabel}
                                                </h4>
                    ...
                    {selectedRubric.type === RUBRIC_TYPES.SINGLE_POINT && getCriteria().map((criterion) => (
                        <div key={criterion.criterion_id || criterion.id} className="bg-slate-50 rounded-xl p-6 border border-slate-200">
                            <div className="mb-4 text-center">
                                <h4 className="font-bold text-lg text-slate-800 mb-1">{criterion.criterion_name || criterion.name}</h4>
                                <div className="inline-block bg-white px-4 py-2 rounded-lg border border-blue-100 text-blue-700 font-medium text-sm shadow-sm">
                                    Target: {criterion.standard || criterion.criterion_description || 'N/A'}
                                </div>
                            </div>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        onChange={(e) => handleComment(`${criterion.criterion_id || criterion.id}_improvement`, e.target.value)}
                                    />
                                </div>
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        onChange={(e) => handleComment(`${criterion.criterion_id || criterion.id}_exceeds`, e.target.value)}
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                            <div className="bg-emerald-50 border border-emerald-100 p-4 rounded-lg flex items-start gap-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-emerald-700">Mark the items achieved by the student in their growth journey.</p>
                                </div>
                            </div>
                            {getItems().map((item) => {
                                ...
                                return (
                                    <div key={itemId}
                                        className={`flex items-center p-4 rounded-xl border-2 transition-all cursor-pointer ${scores[itemId] === 1 ? 'border-emerald-500 bg-emerald-50 shadow-sm' : 'border-slate-100 hover:bg-slate-50'
                                            }`}
                                        onClick={() => handleScore(itemId, scores[itemId] === 1 ? 0 : 1)}
                                    >
                                        <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center mr-4 transition-all ${scores[itemId] === 1 ? 'bg-emerald-500 border-emerald-500 rotate-0' : 'border-slate-300 bg-white -rotate-90'
                                            }`}>
                                            {scores[itemId] === 1 && (
                                        ...
                                        <div className="grow">
                                            <p className={`font-bold text-base ${scores[itemId] === 1 ? 'text-emerald-900' : 'text-slate-700'}`}>{itemLabel}</p>
                                            {itemRequired && <span className="inline-block mt-1 px-2 py-0.5 bg-red-100 text-red-600 text-[10px] font-black rounded tracking-wider">Required Outcome</span>}
                                        </div>
                                    </div>
                                );
                            })}
