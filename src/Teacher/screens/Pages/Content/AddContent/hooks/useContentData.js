...
export const useContentData = (formData) => {
    ...
    const loadQuizzes = useCallback(async () => {
        if (!formData.selectedUnit) return;
        try {
            console.log("Loading quizzes for unit:", formData.selectedUnit);
            setLoading(prev => ({ ...prev, quizzes: true }));
            const res = await contentService.getQuizzesByUnitId(formData.selectedUnit);
            console.log("Quiz API response:", res);
            const quizzes = Array.isArray(res) ? res.map(quiz => ({
