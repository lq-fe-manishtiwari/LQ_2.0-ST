...
import { Search, Eye, ExternalLink, Filter, ChevronDown } from 'lucide-react';
    ...
    const mockData = [
      {
        ...
        semester: 'S1',
        division: 'A',
        registration_link: 'www.learnqoch.com',
  ...
  return (
    <div className="p-0 md:p-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Job Registrations</h2>
          <p className="text-gray-600 mt-1">Manage student registrations for job openings</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-primary-600">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-50 uppercase tracking-wider">Organization</th>
                ...
                <th className="px-6 py-3 text-center text-xs font-medium text-gray-50 uppercase tracking-wider">Pending</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentEntries.length > 0 ? (
                currentEntries.map((item) => (
                  <tr key={item.placement_id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{item.placement_id}</td>
                    <td className="px-6 py-4 text-sm text-gray-500">{item.organization}</td>
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                        {item.pending_registration}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-center">
                      <button
                        onClick={() => navigate(`/teacher/placement/registration/view/${item.placement_id}`)}
          ...
          currentEntries.map((item) => (
            <div key={item.placement_id} className="bg-white rounded-xl shadow-md border border-gray-200 p-5 hover:shadow-lg transition-all">
              <div className="flex justify-between items-start mb-3">
                <div>
                  <p className="font-semibold text-gray-900">{item.organization}</p>
                  <p className="text-sm text-gray-500">{item.job_role}</p>
                  <p className="text-xs text-gray-400 mt-1">{item.placement_id}</p>
                </div>
                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                  {item.pending_registration} Pending
                </span>
              </div>
              ...
              <div className="space-y-2 text-sm text-gray-700 mb-4">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div><span className="font-medium">Division:</span> {item.division}</div>
                </div>
              ...
              <div className="flex justify-end items-center">
                <button
                  onClick={() => navigate(`/teacher/placement/registration/view/${item.placement_id}`)}
                  className="px-4 py-2 rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 transition flex items-center gap-2"
                  title="View"
                >
                  <Eye className="w-4 h-4" />
