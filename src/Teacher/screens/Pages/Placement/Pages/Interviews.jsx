...
export default function Interview() {
  ...
  const loadInterviews = async () => {
    ...
    const mockData = [
      ...
      {
        placement_id: 'PL20250448',
        organization: 'ShubhTest',
        opening_date: '14/07/2025',
        job_role: 'Developer',
        department: 'Testing Grade',
        class: '',
        semester: 'S1',
        division: 'A',
        date: '15/07/2025',
      ...
      {
        placement_id: 'PL20250449',
        organization: 'LearnQoch',
        opening_date: '14/07/2025',
        job_role: 'Java Developer',
        department: 'Testing Grade',
        class: '',
  ...
  const paginatedData = useMemo(() => {
    ...
    if (filters.interviewRound) {
      list = list.filter(item => item.round === filters.interviewRound);
    }
    if (filters.interviewOutcome) {
      list = list.filter(item => item.outcome === filters.interviewOutcome);
    }
  ...
  }, [interviews, searchTerm, currentPage, filters]);
  ...
  const handleView = (id) => {
    navigate(`/teacher/placement/view-interview/${id}`);
  };
    ...
    <div className="p-0 md:p-0">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 sm:gap-0">
        <div>
          ...
          <p className="text-gray-600 mt-1">Manage interview schedules and outcomes</p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-5 mb-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <CustomSelect
              ...
              options={['BCOM', 'Testing Grade', 'BCA', 'BSC']}
            ...
            <CustomSelect
              ...
              onChange={(e) => {
                setFilters(prev => ({ ...prev, interviewRound: e.target.value }));
                resetPage();
              }}
              options={['Technical Interview', 'HR Interview', 'Managerial Interview', 'Final Round', 'Aptitude Test']}
              placeholder="Select Round"
            />
            <CustomSelect
              label="Interview Outcome"
              value={filters.interviewOutcome}
              onChange={(e) => {
                setFilters(prev => ({ ...prev, interviewOutcome: e.target.value }));
                resetPage();
              }}
              options={['Selected', 'Rejected', 'On Hold', 'Pending']}
          ...
          <table className="w-full">
            <thead className="bg-primary-600">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-50 uppercase tracking-wider">Organization</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-50 uppercase tracking-wider">Time</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-50 uppercase tracking-wider">Round</th>
                <th className="px-4 py-3 text-left text-xs font-semibold text-gray-50 uppercase tracking-wider">Outcome</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {currentEntries.map((item, idx) => (
                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-700">{item.round || '-'}</td>
                  <td className="px-4 py-3 text-sm text-gray-700">{item.outcome}</td>
            ...
            <div className="space-y-2 text-sm">
              ...
              {item.round && (
                <div className="flex justify-between">
                  ...
                  <span className="text-gray-900">{item.round}</span>
                </div>
              )}
              <div className="flex justify-between">
                <span className="text-gray-500">Outcome:</span>
                <span className="text-gray-900">{item.outcome}</span>
              </div>
            </div>
