    ...
    return (
      ...
      ${filterType === "absent"
          ...
          ${filterType === "absent" ? "bg-red-200" : "bg-green-200"
            ...
            {/* Teachers Table - TimeTableList Style
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
                    <h3 className="text-lg font-bold text-gray-800">
                        {filterType === 'all' ? 'All Teachers' : filterType === 'present' ? 'Present Teachers' : 'Absent Teachers'}
                    </h3>
                    <div className="text-sm text-gray-600">
                        Total <span className="font-bold text-blue-600">{teachersList.length}</span> staff members
                    </div>
                </div>
                <div className="overflow-x-auto">
                    <table className="w-full min-w-max">
                        <thead className="bg-blue-800 text-white">
                            <tr>
                                <th className="px-4 py-3 sm:px-6 sm:py-4 text-left text-xs font-medium tracking-wider">Staff ID</th>
                                <th className="px-4 py-3 sm:px-6 sm:py-4 text-left text-xs font-medium tracking-wider">Teacher Name</th>
                                <th className="px-4 py-3 sm:px-6 sm:py-4 text-left text-xs font-medium tracking-wider">Department</th>
                                <th className="px-4 py-3 sm:px-6 sm:py-4 text-center text-xs font-medium tracking-wider">In Time</th>
                                <th className="px-4 py-3 sm:px-6 sm:py-4 text-center text-xs font-medium tracking-wider">Out Time</th>
                            ...
                            {teachersList.length === 0 ? (
                                ...
                                teachersList.map((teacher) => (
                                    <tr key={teacher.id} className="hover:bg-gray-50">
                                        <td className="px-4 py-3 sm:px-6 sm:py-4 text-sm text-gray-700">{teacher.staffId}</td>
                                        <td className="px-4 py-3 sm:px-6 sm:py-4 text-sm font-medium text-gray-900">{teacher.name}</td>
                                        <td className="px-4 py-3 sm:px-6 sm:py-4 text-sm text-gray-600">{teacher.department}</td>
                                        <td className="px-4 py-3 sm:px-6 sm:py-4 text-center text-sm text-gray-600">{teacher.checkIn}</td>
                                        <td className="px-4 py-3 sm:px-6 sm:py-4 text-center text-sm text-gray-600">{teacher.checkOut}</td>
                                        <td className="px-4 py-3 sm:px-6 sm:py-4 text-center">
                                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${teacher.status === 'P' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                                                }`}>
                                                {teacher.status === 'P' ? 'Present' : 'Absent'}
                                            </span>
                                        </td>
                                    </tr>
                                ))
