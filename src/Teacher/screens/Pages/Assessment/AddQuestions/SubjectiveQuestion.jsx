...
const SubjectiveQuestion = ({
  ...
  // Initialize data (same as Objective)
  useEffect(() => {
  ...
  const CustomDropdown = ({ id, label, value, options, placeholder, required, onSelect, disabled, loading }) => (
    <div ref={el => (dropdownRefs.current[id] = el)} className="relative">
      ...
      <div
        className={`w-full px-3 py-2 border bg-white cursor-pointer rounded-md min-h-[40px] flex items-center justify-between ${disabled ? 'bg-gray-100 cursor-not-allowed' : 'hover:border-blue-400'}`}
        ...
        <div key={index} className="border border-gray-200 rounded-lg p-6 bg-gray-50 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select value={q.questionType} onChange={e => handleQuestionChange(index, 'questionType', e.target.value)} className="w-full border rounded px-3 py-2 bg-white">
          ...
          <div>
            ...
            <textarea rows="4" value={q.question} onChange={e => handleQuestionChange(index, 'question', e.target.value)} className="w-full border rounded px-3 py-2" placeholder="Enter question..." />
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Model Answer (Optional)</label>
            <textarea rows="3" value={q.modelAnswer} onChange={e => handleQuestionChange(index, 'modelAnswer', e.target.value)} className="w-full border rounded px-3 py-2" placeholder="Enter model answer..." />
          </div>
        </div>
      ))}
