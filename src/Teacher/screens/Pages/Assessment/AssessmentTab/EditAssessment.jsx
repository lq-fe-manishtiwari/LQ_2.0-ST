...
const EditAssessment = ({ showSuccessModal, showWarningModal, userRole }) => {
  ...
  const [state, setState] = useState({
    ...
    rubricType: '',
    testDate: new Date(),
    testLastDate: new Date(),
    timeLimit: '',
    mode: 'ONLINE', // New state
  ...
  // Helper refs for filtering (mostly unused now with hook, but keeping if needed for questions)
  const allBatchesRef = useRef([]);
  // const allAcademicYearsRef = useRef([]); // Removed
  // const allSemestersRef = useRef([]); // Removed
      ...
      try {
        ...
        if (initialAssessmentData) {
          ...
          setState({
            ...
            testDate: ad.test_start_datetime ? new Date(ad.test_start_datetime) : new Date(),
            testLastDate: ad.test_end_datetime ? new Date(ad.test_end_datetime) : new Date(),
    ...
    <div className="max-w-7xl mx-auto p-4 md:p-8 bg-white rounded-xl shadow-lg border border-gray-100">
      ...
      <Formik
        initialValues={{
          ...
          test_date: moment(state.testDate).format("YYYY-MM-DDTHH:mm"),
          test_last_date: moment(state.testLastDate).format("YYYY-MM-DDTHH:mm"),
          ...
          <Form className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                <Field name="test_date" type="datetime-local" className="mt-1 w-full border border-gray-300 rounded-md p-2" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">End Date</label>
                <Field name="test_last_date" type="datetime-local" className="mt-1 w-full border border-gray-300 rounded-md p-2" />
              </div>
              <div>
