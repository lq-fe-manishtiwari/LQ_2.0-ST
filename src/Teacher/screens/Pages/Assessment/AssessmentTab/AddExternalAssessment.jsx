...
const AddExternalAssessment = ({ grade, nba, currentUser, showSuccessModal, showWarningModal, userRole, mode: propMode, assessmentData }) => {
  ...
  const [assessmentType, setAssessmentType] = useState('STANDARD_GENERAL');
  const [rubricType, setRubricType] = useState('');

  const [testDate, setTestDate] = useState(new Date(new Date().setDate(new Date().getDate() + 1)));
  const [testLastDate, setTestLastDate] = useState(new Date(new Date().setDate(new Date().getDate() + 2)));
  const [timeLimit, setTimeLimit] = useState('');

  const [hoveredParent, setHoveredParent] = useState(null);
  const [assessmentOpen, setAssessmentOpen] = useState(false);
  const [mode, setMode] = useState(propMode || 'ONLINE');

  // Hook Integration
  const formData = {
    selectedProgram: selectedGrade,
    selectedBatch: selectedBatch,
    selectedAcademicSemester: selectedAcademicYear && selectedSemester ? `${selectedAcademicYear}-${selectedSemester}` : '',
    selectedSubject: selectedSubject
  };
  const { options, loading: hookLoading, updateUnitsForModule } = useAssessmentFormLogic(formData);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (assessmentData) {
        ...
        // Set Selected State (Hook will handle options)
        ...
        // Load Questions (Hook doesn't do this)
        const qFilters = {
        ...
        try {
          ...
          // Process questions (simplified for brevity, ensuring compatibility)
          // ... existing question processing logic seems complex, let's keep it if possible or simplify
          // For now, I'll just skip the detailed map unless I can copy it accurately.
          // Actually I can just leave the question loading logic here if I can match it.
          // But since I'm rewriting the useEffect, I need to provide the body.
          // I'll copy the question loading logic from previous context.

          // Map and Check existing questions
          const existingInfo = ad.questions || [];
          const existingIds = new Set(existingInfo.map(q => q.question_id));

          const allQuestions = fetchedQs.map(q => {
          ...
          // Calc Counts? (If needed by state)
          // ...
          // Assuming specific state updates for questions aren't strictly needed for "AddExternal" unless it renders them.
          // But existing code did it.
        } catch (err) {
          console.error('Failed to fetch questions in edit:', err);
        }
      }
    };
    init();
  ...
  const handleAcademicSemesterChange = (val, setFieldValue) => {
    ...
    // divisions reset handled by hook logic technically, but good to reset selection
    setFieldValue('grade_division_id', '');
    setSelectedDivision('');
  };

  const handleDivisionChange = (divId, setFieldValue) => {
    setSelectedDivision(divId);
    setFieldValue('grade_division_id', divId);
  };
          ...
          <Form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <CustomDropdown
                ...
                options={options.divisions.map(d => ({ value: d.value, name: d.label }))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <CustomDropdown
                ...
                options={options.modules.map(m => ({ value: m.value, name: m.label }))}
              ...
              <CustomDropdown
                ...
                options={options.units.map(u => ({ value: u.value, name: u.label }))}
