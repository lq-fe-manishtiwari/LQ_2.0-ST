...
const AcademicCalendarDashboard = () => {
  ...
  const renderMobileDateCard = (dateInfo, index) => {
    ...
    if (hasEvents) {
      ...
      // Render blank date card for dates without events
      return (
        <div 
          key={index}
          onClick={() => openAddEventModal(day)}
          className={`bg-white border border-gray-200 rounded-xl p-4 mb-3 cursor-pointer transition-all hover:shadow-md hover:scale-[1.02] hover:border-blue-300 ${
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        <div 
          key={day} 
          onClick={() => openAddEventModal(day)}
          className={`${isExpanded ? 'min-h-[200px]' : 'min-h-[140px]'} border rounded-lg p-2 transition-all hover:shadow-md bg-white cursor-pointer relative overflow-hidden ${
