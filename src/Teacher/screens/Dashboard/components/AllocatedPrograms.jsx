        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div className="flex-1">
              ...
              {/* Quick info - only for non-mentor if needed */}
              {!isMentor && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mt-3">
                  {allocation.batch && <div><span className="text-gray-500">Batch:</span> {allocation.batch.batch_name}</div>}
                  {allocation.semester && <div><span className="text-gray-500">Sem:</span> {allocation.semester.name}</div>}
                  {allocation.division && <div><span className="text-gray-500">Div:</span> {allocation.division.division_name}</div>}
                  {allocation.academic_year && <div><span className="text-gray-500">Year:</span> {allocation.academic_year.name}</div>}
                </div>
              )}
            </div>
            ...
            <div className="flex items-center gap-3">
              {studentCount > 0 && (
                <div className="text-right">
                  <div className="text-sm font-medium text-gray-900">{studentCount}</div>
